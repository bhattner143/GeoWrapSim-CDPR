...
task_filter = mx_task(@()filter, 1/1500); % filter task
task_controller = mx_task(@()controller, 1/1500); % controller task
task_cmd = mx_task(@()cmd_gen, 1/1500); % command generator task
...
while (timenow <= timestart + 15) % runs tasks for 15 seconds                    
...
cmd_current = -0.3 * s;                           % smooth sliding controller
