...
classdef ForwardDynamics < handle
    ...
    methods
        ...
        function [q, q_dot, q_ddot, dynamics] = compute(obj, qp, qp_d, f_active, cable_indices_active, w_ext, dt, model)
            ...
            tau  = - model.L'*f_active;
            q_dd = model.M\(tau - model.C - model.G - model.W_e - w_ext);
        ...
        function [q, q_dot, q_ddot, dynamics] = compute2(obj, qp, qp_d, f_active, cable_indices_active, w_ext, dt, model)
            n_vars = model.numDofVars;
            n_dofs = model.numDofs;
            
            % The procedure to perform the forward dynamics
            [q, q_dot, q_ddot] = ForwardDynamics.eom2( model, qp, qp_d, f_active, dt, cable_indices_active, w_ext);

%             model.update(q, q_dot,zeros(n_dofs,1),zeros(model.numDofs,1));
            model.update(q, q_dot,q_ddot,zeros(model.numDofs,1));
    ...
    methods (Static, Access = private)
        ...
        function [q, q_d, q_dd] = eom2(model, qp, qp_d, f_active, dt, cable_indices_active, w_ext)
            n_vars = model.numDofVars;
            n_dofs = model.numDofs;
            model.update(qp, qp_d, zeros(n_dofs, 1), w_ext);
        ...
        function y_dot = eom3(~, y, model, f_active, cable_indices_active, w_ext)
            n_vars = model.numDofVars;
            n_dofs = model.numDofs;
            q = y(1:n_vars);
            q_dot = y(n_vars+1:length(y));
