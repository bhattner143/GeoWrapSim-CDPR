...
classdef FileOperations2
    methods(Static)
        ...
        function CompleteTrajectoryGenerator(refTrajectory, file_name, operation_mode)
            ...
            fid = fopen([file_name,'.traj'],operation_mode);
            for i = 1:length(t)-1
        ...
        % A function to convert the reference operational space trajectory into a otraj file
        % (file_name).
        function CompleteOperationalTrajectoryGenerator(operationalRefTrajectory, file_name, operation_mode)
            t = operationalRefTrajectory.timeVector;
            n_y = length(operationalRefTrajectory.y{1});
            fid = fopen([file_name,'.otraj'],operation_mode);
            for i = 1:length(t)-1
                y_tmp = operationalRefTrajectory.y{i};
                v_tmp = operationalRefTrajectory.y_dot{i};
                a_tmp = operationalRefTrajectory.y_ddot{i};
                % First the time
                str = ['t,',num2str(t(i)),',y,'];
                % Now add the joint pose
                for j = 1:n_y
        ...
        % A function to convert an otraj file (file_name) into a trajectory.
        function trajectory = ReadCompleteOperationalTrajectory(file_name,n_y)
            ...
            while ~feof(fid)
                ...
                % Test if there were multiple q variables
                CASPR_log.Assert(isempty(strfind(l_split_y{2},'y')),'Only one y should occur on each line');
                % Split the outcome using v
