...
%       AddNoise2Length             ->  add_noise to length
%       AddNoise2JtPosAndVel        ->  add_noise to jt pos and vel
%       NoiseCoeff                  ->  noise_coeff_value
...
classdef ControllerSimulatorOptions < handle
    properties
        ...
        add_noise_l                         % add noise to the length encoders
        add_noise_q_q_dot                   % add noise to the joint feedback and velocity
        noise_coeff_value                   % noise coeffecient value
    ...
    methods
        ...
        function defaultOptions(obj)
            ...
            obj.add_noise_l                     =   false;
            obj.add_noise_q_q_dot               =   false;
            obj.noise_coeff_value               =   0;
        ...
        function inputInterpretation(obj, input_category, input_value)
            ...
            if (strcmp(input_category, 'AddNoise2Length'))
                if (strcmp(input_value, 'true'))
                    obj.add_noise_l     =   true;
                else
                    obj.add_noise_l    =   false;
                end
            end
            if (strcmp(input_category, 'AddNoise2JtPosAndVel'))
                if (strcmp(input_value, 'true'))
                    obj.add_noise_q_q_dot     =   true;
                else
                    obj.add_noise_q_q_dot     =   false;
                end
            end
            if obj.add_noise_l || obj.add_noise_q_q_dot
                if (strcmp(input_category, 'NoiseCoeff'))
                      obj.noise_coeff_value = input_value;
                end
            end
