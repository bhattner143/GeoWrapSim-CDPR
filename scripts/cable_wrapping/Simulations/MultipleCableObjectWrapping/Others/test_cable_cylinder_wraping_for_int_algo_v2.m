...
% for obj_num_present = 1:4
    ...
    if obj_num_present == 1
        ...
        line{1} = object_connection_map(obj_num_present).object.C;
%         line{2} = alpha_prev(1,:)';
        line{2} = object_connection_map(end).object.D;
    end
...
%                 objects(obj_num_present).object.child = objects(5).object.name;
    ...
    % Update the connection map
    % Keep the object closest to  current object (obj_num_present)
    object_connection_map = [object_connection_map(1:temp_index + 1) object_connection_map(end)];
    
    % Update the cwg wrt to current object connection map
    ...
    %
    for jj = 2:length(object_connection_map)
