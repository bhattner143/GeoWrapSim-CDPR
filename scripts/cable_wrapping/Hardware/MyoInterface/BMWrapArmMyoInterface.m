...
classdef BMWrapArmMyoInterface < CableActuatorInterfaceBase
    properties (Access = private)        
        ...
        joint_cmd_pub;              % joint command publisher
        joint_cmd_msg;              % joint command message 
        
        pose_sub;                   % pose subscriber
        pose_msg;                   % pose message
        
        angle_sub                 % angle subscriber
        angle_msg                 % angle message
        
%         length_fb_sub               % length feedback subscriber
%         length_fb_msg               % length feedback subscriber
    ...
    properties (Access = private, Constant = true)  
        ...
        angle_topic = '/as5047p';
        angle_msg_type = 'std_msgs/String';

    end
    
    methods (Access = public)       
        function interface = BMWrapArmMyoInterface(ROS_MASTER_URI, ROS_IP)
    ...
    methods (Access = private)
        ...
        function createSubscriber(obj)
            try
                ...
                obj.angle_sub = rossubscriber(obj.angle_topic, obj.angle_msg_type);
                
%                 %getting the length feedback from the FPGA board
%                 obj.length_sub = rossubscriber(obj.length_topic, obj.length_msg_type, @obj.sub_callback);
            catch
...
%                 rosinit();
        ...
        % subscribe to angle encoder data and read angle data
        function angle_array = angleEncoderFeedbackRead(obj) 
            obj.angle_msg = receive(obj.angle_sub,10);
            while isempty(obj.angle_msg)
                obj.angle_msg = receive(obj.angle_sub,10);
            end        
            angle_string = obj.angle_msg.Data;   
            angle_array  = str2double(split(angle_string,','));
            obj.angle_msg = [];
        end
        ...
        function sub_callback_angle(obj, ~, msg)
            obj.angle_msg = msg;
        end
