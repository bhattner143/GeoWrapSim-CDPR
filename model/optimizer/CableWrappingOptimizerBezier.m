...
classdef CableWrappingOptimizerBezier < CableWrappingMotionSimulatorBase
...
%         ic_type                    = 'defined';
        ...
        % For bezier and nurbs surface
        
        %Teapot handle inside top wrapping
        % bounds={[0.0,       0.0,      -0.3,   -0.6],[1,         1,         0.0,    0];% Teapot cover
        %         [0.0,       0.0,      -0.3,   -0.6],[1,         1,         0.0,    0];% Teapot body
        %         [0.0,       0.1,       0,   -0.2], [1,          1,         1,    0]}% Teapot handle

         %Teapot handle inside bottom wrapping
        bounds={[0.0,       0.0,      -0.3,   -0.6],  [1,         1,         0.0,    0];% Teapot cover
                [0.0,       0.0,      -0.1,    -0.1], [1,        1,         0.0,    0];% Teapot body
                [0.0,       0.5,       0,     -0.1],  [1,        1,         0.5,    0]}% Teapot handle
...
%           Curve General
    ...
    methods
        % Constructors
        function cwo = CableWrappingOptimizerBezier(model_config, wrap_optimizer_with_gen_int_det)
...
%              k   = linspace(0.5,1.5, 100);%almond cable 1,2
        ...
        function run(cwo,lb,ub,tol, wrap_optimizer_with_gen_int_det, ...
            ...
            % algorithm
            if strcmp(class(wrap_optimizer_with_gen_int_det), 'CWOptWithGenIntDetBezier')
                cwo.lb           = lb;
                cwo.ub           = ub;
                cwo.tol          = tol;
                ...
                for i  =  cable_indices
                    ...
                    if i == 2                        
                        ...
                        lb_obs = cwo.bounds(cwo.wo_int_det_model.surface_select_array,1);
                        ub_obs = cwo.bounds(cwo.wo_int_det_model.surface_select_array,2);

                        if isempty(b_prev_array) & isempty(k_prev_array) & isempty(bkobs_opt_prev_array)
                            bk_all_obj_prev_this = [];
                        else
                            b_prev_this_cable   = b_prev_array(i);
                            k_prev_this_cable   = k_prev_array(i);
                            bkobs_opt_prev_this = bkobs_opt_prev_array{i};
                            u_A_cable_2 = cwo.model_config.cable_info.cable{2}.helixParams.u0_b;
                            v_A_cable_2 = cwo.model_config.cable_info.cable{2}.helixParams.v0_b;
                            bk_all_obj_prev_this = [bkobs_opt_prev_this'; [u_A_cable_2,v_A_cable_2,b_prev_this_cable,k_prev_this_cable]]';
                        ...
                        cwo.wo_int_det_model.run_cwg_with_int_detection(P_g, A_b,...
                                                    lb_link, ub_link,...
                        ...
                        switch cwo.wrapping_case{i}
                            case 'multi_wrapping'
                                ...
                                for ii = 2:length(wrap_optimizer_with_gen_int_det.object_connection_map)-1
                                    if strcmp(wrap_optimizer_with_gen_int_det.object_connection_map(ii).object.category,'obstacle')
                                        %Obstacle wrapping
                                        obs_surface_select =  wrap_optimizer_with_gen_int_det.object_connection_map(2).object.number+1;

                                        cwo.bk_obs = cwo.wo_int_det_model.bk_obs_obj(:,obs_surface_select-1);                        
                                        ...
                                        wrap_model_config.UpdateObstacleHelicalWrappingParams(cwo.bk_obs, check_helix,i,obs_surface_select );%For updating params on obstacle
                                    elseif strcmp(wrap_optimizer_with_gen_int_det.object_connection_map(ii).object.category,'link')

                                        %self wrapping
                                        cwo.b = cwo.wo_int_det_model.bk_obs_obj(3,wrap_optimizer_with_gen_int_det.object_connection_map(ii).object.number);
                                        cwo.k = cwo.wo_int_det_model.bk_obs_obj(4,wrap_optimizer_with_gen_int_det.object_connection_map(ii).object.number);
                                ...
                                ls = 0;
                                for obj_in_cm = 1:length(cwo.wo_int_det_model.object_connection_map)-1
                                    if obj_in_cm == 1
                                        C_obj = cwo.wo_int_det_model.object_connection_map(obj_in_cm).object.P_g;
                                    else
                                        T_g_obj = cwo.wo_int_det_model.object_connection_map(obj_in_cm).object.T_g_obj;
                                        C_obj   = T_g_obj*[cwo.wo_int_det_model.object_connection_map(obj_in_cm).object.C_obj' 1]';
                                        C_obj   = C_obj(1:3);
                                    end

                                    if obj_in_cm == length(cwo.wo_int_det_model.object_connection_map)-1
                                        T_g_obj = cwo.wo_int_det_model.object_connection_map(obj_in_cm+1).object.T_g_obj;
                                        D_obj   = T_g_obj*[cwo.wo_int_det_model.object_connection_map(obj_in_cm+1).object.A_obj' 1]';
                                        D_obj   = D_obj(1:3);
                                    else
                                        T_g_obj = cwo.wo_int_det_model.object_connection_map(obj_in_cm+1).object.T_g_obj;
                                        D_obj   = T_g_obj*[cwo.wo_int_det_model.object_connection_map(obj_in_cm+1).object.D_obj' 1]';
                                        D_obj   = D_obj(1:3);
                                    end                                    
                            ...
                            case 'no_wrapping'
                                ...
                                cwo.bopt_array(cable_num,1) = cwo.b;
                                cwo.kopt_array(cable_num,1) = cwo.k;
                                ...
                                % the helix curve
                                check_helix = 1; %generate helix with new params
                                wrap_model_config.UpdateHelicalWrappingParams(cwo.b,cwo.k, check_helix,cable_num);

                                % Update the helix parameters wrt new bk_obs and generate
                                % the helix curve
                                check_helix = 1; %generate helix with new params
                                wrap_model_config.UpdateObstacleHelicalWrappingParams(cwo.bk_obs, check_helix,cable_num);%For updating params on obstacle
                                ...
                                %Straight part length
                                ls = norm(optParam.P_g(1:3) - optParam.A_g(1:3));
...
%             F = (P_b(1)+t*PA(1)).^2.*((P_b(1)+t*PA(1))./r+3*pi/2).obstacle_detection_info^2 + (P_b(3)+t*PA(3)).^2-r.^2;
%               psi = (pi/2 - (P_b(1)+t*PA(1))./r);
    ...
    %                                 save_pts_intersection = pts_intersection;
            ...
            elseif strcmp(cwo.model_config.obsHelixParams.obstacle_surface_param.surface_name,'nurbs_and_bezier')
                ...
                obstacle_detection_state = zeros(length(cable_indices),1);
                t = zeros(length(cable_indices),1);
                obstacle_surface_hit = zeros(length(cable_indices),1);
                for surface_select = 1:length(cwo.model_config.obstacle_surf_data_struct)
                    for cable_index =  cable_indices
                    %Only checking for second cable
                    if surface_select == 2
                        tol_fval = 1e-3;
                        tol_dist = [1e-2,1e-2,1e-2];
                    elseif surface_select == 3
                        tol_fval = 1e-4;
                        ...
                        if cable_index == 2
                            solve_t_info = cwo.solve_t(cable_index, surface_select, tol_dist);
                            
                            if isempty(solve_t_info.t_cropped) == false 
                                if (solve_t_info.t_opt < 1) && (solve_t_info.fval <= tol_fval)
                                    obstacle_detection_state(cable_index) = 1;
                                    t(cable_index)                        = solve_t_info.t_opt;
                                    obstacle_surface_hit(cable_index)     = surface_select;
                                end
                            end
                        end
                    end
                ...
                obstacle_detection_info.t                         = t;
                obstacle_detection_info.obstacle_detection_state =  obstacle_detection_state;
                obstacle_detection_info.obstacle_surface_hit     = obstacle_surface_hit;
            end    
        end
        %%
...
%             [alpha_t_b,  ~, ~] = obj_cable_wrapping.GenNumCompHelixCurve(param, bk_obs, tspan); 
            if strcmp(obj_cable_wrapping.obsHelixParams.obstacle_surface_param.surface_name,'nurbs_and_bezier')
                % t = tic;
                [alpha_obs_t_o, ~] = obj_cable_wrapping.model_geodesic.GenObstacleNumCompHelixCurve(param, bk_obs, tspan);
                % toc(t)
            else
                % t = tic;
            ...
            f =  f_start + f_end;%+l_tot.^2;

        end
        % Cable obstacle wrapping obstacle objective function
        function f = ObjFuncForCableWrappingNURBSObstacleCASPR(cwo, bk_obs, cable_num, obstacle_select)

            obj_cable_wrapping = cwo.model_config;
            gen_helix_curve = 0;
%             obj_cable_wrapping.UpdateHelicalWrappingParams(b,k,gen_helix_curve,cable_num);
            % t = tic;
            obj_cable_wrapping.UpdateObstacleHelicalWrappingParams(bk_obs,gen_helix_curve,cable_num,obstacle_select);
            % toc(t)    
            optPram   = struct();
...
%             [alpha_t_b,  ~, ~] = obj_cable_wrapping.GenNumCompHelixCurve(param, bk_obs, tspan); 
        ...
        % Cable obstacle wrapping obstacle objective function
        function l_tot = ObjFuncForCableWrappingObstacleCASPRv2(cwo, bk_obs, cable_num)
...
%             [alpha_t_b,  ~, ~] = obj_cable_wrapping.GenNumCompHelixCurve(param, bk_obs, tspan); 
            ...
            dalpha1_k = diff(alpha_obs_t_o(:,1));
            dalpha2_k = diff(alpha_obs_t_o(:,2));
            dalpha3_k = diff(alpha_obs_t_o(:,3));
    
            l_w = sum(sqrt(dalpha1_k.^2 + dalpha2_k.^2 + dalpha3_k.^2)); % Arc length. Linear aprox.  

         
            % Total length
            l_tot = (l_w + l_AC + l_PD).^2;
           

        end
        % nonlinear constraints
        function [f,f_eq] = ConstraintTangent(cwo, bk_obs, cable_num)
            obj_cable_wrapping = cwo.model_config;
            gen_helix_curve = 0;
%             obj_cable_wrapping.UpdateHelicalWrappingParams(b,k,gen_helix_curve,cable_num);
...
%             [alpha_t_b,  ~, ~] = obj_cable_wrapping.GenNumCompHelixCurve(param, bk_obs, tspan); 
            ...
            f_eq(1) = delta_alpha_obs_t_start_unit(1) - AC_unit(1); 
             f_eq(2) = delta_alpha_obs_t_start_unit(2) - AC_unit(2);
              f_eq(3) = delta_alpha_obs_t_start_unit(3) - AC_unit(3);
            f_eq(4) = delta_alpha_obs_t_end_unit(1)   - DP_unit(1); 
            f_eq(5) = delta_alpha_obs_t_end_unit(2)   - DP_unit(2); 
            f_eq(6) = delta_alpha_obs_t_end_unit(3)   - DP_unit(3); 
        end
        ...
        function [optParam, f] = GenOptParamsAndUnitVectorsForNumGeodesic(cwo,cable_num, gen_curve_again)
            ...
            optParam.A_g                = obj_cable_wrapping.cable_info.cable{cable_num}.A_g;
            ...
            optParam.A_b                = obj_cable_wrapping.cable_info.cable{cable_num}.A_b;
        end
        ...
        function lw = ComputeGeodesicCableWrapLengths(cwo,cable_index)
            switch cwo.wrapping_case{cable_index}
                ...
                case 'obstacle_wrapping'
                    ...
                    if strcmp(cwo.model_config.obsHelixParams.obstacle_surface_param.surface_name,'nurbs_and_bezier')
                        [alpha_t, ~]     = cwo.model_config.model_geodesic.GenObstacleNumCompHelixCurve(param, bk_obs_opt);
                    else
                ...
                case "multi_wrapping"
                    lw = 0;
                    for i = 2:length(cwo.wo_int_det_model.object_connection_map)-1
                        lw = lw + cwo.wo_int_det_model.object_connection_map(i).object.lw; 
                    end
        ...
        function solve_t_info = solve_t(cwo, cable_index, surface_select, tol_dist)
            ...
            R_all(:,:,1) = obstacle_surface_param.x_cir_o(50*surface_select-49:50*surface_select,:);
            R_all(:,:,2) = obstacle_surface_param.y_cir_o(50*surface_select-49:50*surface_select,:);
            R_all(:,:,3) = obstacle_surface_param.z_cir_o(50*surface_select-49:50*surface_select,:);
            
            x_sel_o = obstacle_surface_param.x_cir_o(50*surface_select-49:50*surface_select,:);
            y_sel_o = obstacle_surface_param.y_cir_o(50*surface_select-49:50*surface_select,:);
            z_sel_o = obstacle_surface_param.z_cir_o(50*surface_select-49:50*surface_select,:);
            ...
            T_g_o = cwo.model_config.obstacle_surface_param.TransformationMatrices.T_g_o;
            ...
            try
                if isempty(t_x) == false
                    L = @(t) P_o(1:3)+ t.*(A_o(1:3) - P_o(1:3));
                    cwo.L = L;
    
                    object_part = cwo.model_config.obstacle_surface_param.nurbs_and_bezier(surface_select).object_part;
                    R           = @(u,v) cwo.model_config.model_geodesic.evaluateNURBS_curve(u,v,object_part);
                    cwo.R       = R;
                    ...
                    func = @(x) cwo.obj_function_line_int_surf(x(1),x(2),x(3)); 
