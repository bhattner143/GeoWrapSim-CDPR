...
%         ub_obs = [0,      0,   0,      2];
        ...
        bounds={[0.0,       0.0,      -0.3,   -0.6],[1,         1,         0.0,    0];% Teapot cover
                ...
                [0.0,       0.0,       0.0,   -0.3],[1,         1,         0.4,    0]}% Teapot handle
...
%              k   = linspace(0.5,1.5, 100);%almond cable 1,2
        ...
        %                 cwo.optimization_info(i).ub      = ub;
            ...
            %% One wrapping at a time implementation (old one)
                ...
                wrapping_info           = cwo.wrap_info;
                obstacle_detection_info = cwo.obstacle_detection_info;
                
                % Only applicable for nurbs and bezier surfaces
                if strcmp(cwo.model_config.obstacle_surface_param.surface_name,...
                        'nurbs_and_bezier')
                    obstacle_surface_hit = obstacle_detection_info.obstacle_surface_hit;
                end
                ...
                for i =  cable_indices
                    ...
                    if wrapping_info.wrap_state(i) == 0
                        cwo.wrapping_case{i} = 'self_wrapping';
                    elseif wrapping_info.wrap_state(i) == true  && obstacle_detection_info.obstacle_detection_state(i) == true
                        cwo.wrapping_case{i} = 'obstacle_wrapping';
                    elseif wrapping_info.wrap_state(i) == 1 && obstacle_detection_info.obstacle_detection_state(i) == 0
                        cwo.wrapping_case{i} = 'no_wrapping';
                    end
                    opttime_obs_cab_int = toc(t_obs_cab_int);
                    ...
                    switch cwo.wrapping_case{i}
                        case 'self_wrapping'
                            ...
                            t_update_link_param_obstacle = tic;
                            wrap_model_config.UpdateHelicalWrappingParams(b,k, check_helix,cable_num);
                            opttime_update_param = toc(t_update_link_param_obstacle);
                        ...
                        case 'obstacle_wrapping'
                            ...
                            if strcmp(cwo.model_config.obstacle_surface_param.surface_name,...
                                ...
                                obstacle_select = obstacle_surface_hit(i);

                                if isempty(b_prev_array) == 1 || isempty(k_prev_array) == 1 || isempty(bkobs_opt_prev_array)||strcmp(cwo.ic_type,'defined')
                                    % Select the obstacle thats been hit

                                    % Select its bounds
                                    lb_obs = cwo.bounds{obstacle_select,1};
                                    ub_obs = cwo.bounds{obstacle_select,2};
                            ...
                            cwo.bk_obs0 = ic_bkobs;
                            %Apply wrap optmizer on the obstacle
                            t_eval_obs_ob_func = tic;
                            if strcmp(cwo.model_config.obstacle_surface_param.surface_name,...
                                    'nurbs_and_bezier')
                                f_init = cwo.ObjFuncForCableWrappingNURBSObstacleCASPR(ic_bkobs, cable_num,obstacle_select);  
                                opttime_eval_obs_ob_func = toc(t_eval_obs_ob_func);

                                % f_init_2 = cwo.ConstraintTangent(ic_bkobs, cable_num);
                                % Define the obj function handle for wrapping around
                                % the obstacle
                                objfun_obs = @(bk_obs)cwo.ObjFuncForCableWrappingNURBSObstacleCASPR(bk_obs, cable_num,obstacle_select);
                                % constraint = @(bk_obs)cwo.ConstraintTangent(ic_bkobs, cable_num);
                            else
                                 f_init = cwo.ObjFuncForCableWrappingObstacleCASPR(ic_bkobs, cable_num); 
                                 opttime_eval_obs_ob_func = toc(t_eval_obs_ob_func);
                                 objfun_obs = @(bk_obs)cwo.ObjFuncForCableWrappingObstacleCASPR(bk_obs, cable_num);
                            ...
                            t_fmincon_obstacle = tic;
                            [bk_obs, fval, exitflag, output] = fmincon(objfun_obs,ic_bkobs,[],[],[],[], lb_obs, ub_obs, [], cwo.options);
                            % [bk_obs, fval, exitflag, output] = lsqnonlin(objfun_obs,ic_bkobs,lb_obs, ub_obs, [],[],[],[],[], cwo.options);
                            opttime = toc(t_fmincon_obstacle);

                            % Update the helix parameters wrt new bk_obs and generate
                            % the helix curve
                            t_update_obs_param_obstacle = tic;
                            check_helix = 1; %generate helix with new params
                            wrap_model_config.UpdateObstacleHelicalWrappingParams(bk_obs, check_helix,cable_num,obstacle_select);%For updating params on obstacle
                            opttime_update_param = toc(t_update_obs_param_obstacle);
    ...
    %                 cwo.optimization_info(i).probDef   =  prob;
                    ...
                    cwo.optimization_info(i).f_opt   = fval;
                    cwo.optimization_info(i).opttime = opttime;
                    cwo.optimization_info(i).opttime_update_param = opttime_update_param;
                    cwo.optimization_info(i).opttime_obs_cab_int  = opttime_obs_cab_int;
