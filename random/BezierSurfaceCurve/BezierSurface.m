classdef BezierSurface < handle
    properties
        P;                  % Control points of the Bezier surface
        n;                  % Degree of the Bezier surface
        num_ctrl_pts;       % Num control points
    ...
    methods
        function obj = BezierSurface(P, n, u0, tspan, u, v)

            if nargin == 4
                n_meshgrid = 64;
                u = linspace(0, 1, n_meshgrid)';
                v = linspace(0, 1, n_meshgrid)';
            end
            % Constructor
            ...
            [obj.u, obj.v] = meshgrid(obj.u, obj.v);



            obj.surface_param.surface_name    = 'bezier';

            obj.surface_param.P               = P;
            obj.surface_param.P_square        = reshape(obj.P, obj.n+1,obj.n+1,[]);
            obj.surface_param.n_deg           = n;
            obj.surface_param.num_ctrl_pts    = n + 1;
        ...
        function alpha = GenObstacleNumCompHelixCurve(obj, bk_tilde_obs, tspan)
            ...
            u_surf = obj.surface_param.u(1,:)';
            v_surf = obj.surface_param.v(:,1)';
            
            % Patch boundaries, u fixed and v varying
            alpha_boundary = zeros(length(u_surf), 3);
            l = 1;
            for i = [1 length(u_surf)]
                k = 1
                for j = 1:length(v_surf)
                    alpha_boundary(k,:) = obj.EvaluateBezzierSurfacePoint(u_surf(i), v_surf(j));
                    k = k+1;
                end
                obj.alpha_boundary{l} = alpha_boundary;
                l = l + 1;
            end
            ...
            for i = [1 length(u_surf)]
                k = 1
                for j = 1:length(v_surf)
                    alpha_boundary(k,:) = obj.EvaluateBezzierSurfacePoint(u_surf(j), v_surf(i));
                    k = k+1;
                end
                obj.alpha_boundary{l} = alpha_boundary;
                l = l + 1;
