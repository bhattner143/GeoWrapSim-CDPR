    ...
    methods
        ...
        function run(obj, ref_trajectory, q0, q0_dot, q0_ddot)
            ...
            while (1 <= t && t <= length(obj.timeVector) && ~obj.pre_mature_termination)
                ...
                if (current_time >= obj.ctrlTrajectory.timeVector(obj.ctrl_counter + 1) - obj.rounding_error_guard && ~obj.pre_mature_termination)
                    ...
                    % data. This function update the controller fb q and
                    % q_dot coming from the FK or from environment
                ...
                if (current_time < tf && ~obj.pre_mature_termination)
                    ...
                    % do forward dynamic update for generating 
        ...
        function cableLengthFeedbackUpdate(obj)
            if (obj.simopt.use_absolute_encoder)
                ...
                if (obj.simopt.add_noise_l)
                    % add noise
                    obj.l	=   obj.modelTrue.cableLengths+obj.simopt.noise_coeff_value*randn(obj.modelTrue.numCables,1);
                else
                    obj.l	=   obj.modelTrue.cableLengths;
                end
            else
        ...
        function controlCommandUpdate(obj, ctrl_delta_t)
            ...
            % This method selects whther feedback information needs to be
            % taken from the FK or directly from the environment
        ...
        function controllerFeedbackUpdate(obj, ctrl_delta_t)
            if (obj.simopt.use_FK_in_controller)
                ...
                [obj.q_fk_prev_ctrl, obj.q_d_fk_prev_ctrl, ~] = ...
                    obj.fkSolver.compute(obj.l, obj.l_fk_prev_ctrl, ...
                    obj.q_fk_prev_ctrl, ...
                    obj.q_d_fk_prev_ctrl, ...
                    ctrl_delta_t,...
                    obj.model.cableModel.cableIndicesActive);
            ...
            elseif (obj.simopt.use_ob_state_estimation)
                ...
                % environment
                if obj.simopt.add_noise_q_q_dot
                    obj.ctrlTrajectory.q{obj.ctrl_counter}     =   obj.trajectory.q{obj.sim_counter}+obj.simopt.noise_coeff_value*randn(obj.modelTrue.numDofs,1);
                    obj.ctrlTrajectory.q_dot{obj.ctrl_counter} =   obj.trajectory.q_dot{obj.sim_counter}+obj.simopt.noise_coeff_value*randn(obj.modelTrue.numDofs,1);
                else
