...
%             value = obj.q_ddot;
        ...
        %This method takes care of / and \ inconsistency if occurs 
        %in the file path between mac and windows.
        
        function MacErrorHandler(obj,dir_path)
            try 
                rmdir(dir_path, 's');
            catch ME
                disp('Mac forward back-slash inconsistency detected in the file path')
                disp('Replacing "\" with "/"')
                
                if contains(dir_path,'\')
                    dir_path = strrep(dir_path,'\','/');
                end
            ...
            if (compile_bodies || compile_cables || compile_opspaces)
                ...
                if (compile_cables)
                    if (exist(c_folder, 'dir'))  
                        obj.MacErrorHandler(cables_base_folder)
                    end
                    mkdir(c_folder);                
                    CASPR_log.Info('Start Body Compilations...');
                ...
                if (compile_opspaces)
                    if (exist(op_folder, 'dir'))
                         obj.MacErrorHandler(opset_base_folder)
                    end
                    mkdir(op_folder);                
                    CASPR_log.Info('Start Body Compilations...');
